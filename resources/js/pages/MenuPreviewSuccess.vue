    ...
    <div class="min-h-screen bg-gradient-to-b from-[#FAEDCD] via-white to-white font-sans flex flex-col relative">
        ...
        <header class="bg-[#3B2314] text-[#FAEDCD] py-5 shadow-md sticky top-0 z-40">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-center">
                <h1 class="text-xl font-black tracking-widest uppercase">Zunoi Caffe (Preview)</h1>
            </div>
        </header>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 items-start">
                ...
                <div class="space-y-5">
                    ...
                    <div class="bg-white p-5 rounded-2xl shadow-sm border border-[#D4A373]/10 text-center relative overflow-hidden">
                        ...
                        <h2 class="text-lg font-black text-[#3B2314] tracking-tight">Pesanan Terkirim! (Simulasi)</h2>
                        <p class="text-[10px] text-gray-400 font-bold mt-0.5">ID PESANAN DEMO: #{{ mockOrderId }}</p>
                        ...
                        <div class="mt-4 pt-3 border-t border-gray-100 grid grid-cols-2 gap-2 text-left text-[10px] text-gray-500 font-bold">
                            <div>
                                ...
                                <span class="text-[#3B2314] text-xs font-black">{{ customer_name || 'Pelanggan Demo' }}</span>
                            </div>
                            <div>
                                <span class="block text-gray-400 font-normal">Meja / Tipe</span>
                                <span class="text-[#3B2314] text-xs font-black">
                                    Meja Preview ({{ order_type === 'dine_in' ? 'Dine In' : 'Takeaway' }})
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="payment_method === 'qris_tokopay'" class="bg-amber-50/50 text-[#3B2314] border border-[#D4A373]/20 p-4.5 rounded-xl text-[10px] font-black leading-relaxed flex items-start gap-2.5">
                        ...
                        <span>Notifikasi WhatsApp akan dikirimkan otomatis jika pembayaran QRIS Anda sukses terkonfirmasi. (Dinonaktifkan dalam mode preview)</span>
                    </div>

                    <!-- Back to Menu Button -->
                    <Link :href="'/dashboard/menu-preview'" class="block w-full bg-[#3B2314] text-[#FAEDCD] py-4 rounded-xl font-black text-center hover:scale-[1.02] active:scale-95 transition text-xs uppercase tracking-wider shadow-md">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-[#D4A373]/10 relative">
                    ...
                    <div class="space-y-3.5 mb-5">
                        <div v-for="item in parsedItems" :key="item.id" class="flex justify-between items-start text-xs">
                            <div class="max-w-[70%]">
                                <span class="font-black text-[#3B2314]">{{ item.name }}</span>
                    ...
                    <div class="space-y-2.5">
                        <div class="flex justify-between text-[10px] text-gray-500 font-bold">
                            ...
                            <span class="text-[#3B2314] uppercase">
                                {{ payment_method === 'cashier' ? 'Bayar di Kasir' : payment_method === 'qris_tokopay' ? 'QRIS Otomatis' : 'QRIS Manual' }}
                            </span>
                        </div>
                        <div class="flex justify-between items-center text-xs pt-1.5">
                            ...
                            <span class="text-base font-black text-[#D4A373]">
                                Rp {{ parseInt(total_price).toLocaleString('id-ID') }}
                            </span>
                        </div>
                    </div>
                </div>
