    ...
    router.post(
        ...
        },
        {
            ...
            onError: (errors: any) => {
                ...
                triggerToast(
                    firstError || 'Gagal menambahkan barista.',
                    'error',
                );
            },
        },
...
return;
    ...
    router.put(
        `/api/staff/${selectedBarista.value.id}`,
        {
            name: nameInput.value,
            email: emailInput.value,
        ...
        },
        {
            ...
            onError: (errors: any) => {
                const firstError = Object.values(errors)[0] as string;
                triggerToast(
                    firstError || 'Gagal memperbarui barista.',
                    'error',
                );
            },
        },
    ...
    triggerConfirm(
        ...
        () => {
            router.delete(`/api/staff/${barista.id}`, {
                onSuccess: () => {
                    triggerToast(
                        'Akun Barista berhasil dihapus secara permanen!',
                        'success',
                    );
                },
                onError: () => {
    ...
    <ZunoiAdminLayout>
        <div class="mx-auto max-w-7xl space-y-8 text-gray-800">
            ...
            >
                ...
                <div class="z-10">
                    ...
                    <p class="mt-1 text-xs text-gray-300">
                        Tambahkan akun staf baru, edit informasi login barista,
                        atau hapus akses login ke kasir POS kafe Anda.
                    </p>
                </div>
                <button
                    @click="openAddModal"
                    class="z-10 mt-4 shrink-0 rounded-xl bg-[#D4A373] px-4 py-2.5 text-xs font-black text-[#3B2314] shadow-md transition hover:scale-[1.02] hover:bg-[#FAEDCD] active:scale-95 md:mt-0"
                >
                    + Tambah Barista Baru
                </button>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
                <div
                    class="flex items-center gap-4 rounded-xl border border-[#D4A373]/20 bg-white p-6 shadow-sm"
                >
                    <div
                        class="flex h-12 w-12 items-center justify-center rounded-lg bg-amber-50 text-[#D4A373]"
                    >
                        <svg
                    ...
                    <div>
                        ...
                        <h4 class="text-xl font-extrabold text-[#3B2314]">
            ...
            >
                <div class="border-b border-gray-100 p-6">
                    <h3 class="text-md font-bold text-[#3B2314]">
                        Daftar Staf Barista Aktif
                    </h3>
                    <p class="mt-0.5 text-xs text-gray-400">
                        Berikut adalah akun staf yang diizinkan untuk mengelola
                        transaksi POS dan kasir.
                    </p>
                </div>

                <!-- Empty State -->
                <div
                    v-if="baristas.length === 0"
                    class="py-16 text-center text-gray-400"
                >
                    <div
                        class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full border bg-gray-50 text-gray-400"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                    ...
                    <p class="text-xs font-bold">
                        Belum ada akun Barista terdaftar.
                    </p>
                    <p class="mt-1 text-[10px] text-gray-400">
                        Silakan klik "+ Tambah Barista Baru" di atas untuk
                ...
                <div v-else class="overflow-x-auto">
                    <table class="w-full border-collapse text-left">
                        <thead>
                            <tr
                                class="border-b border-gray-100 bg-gray-50/70 text-xs font-extrabold tracking-wider text-[#3B2314] uppercase"
                            >
                        ...
                        >
                            <tr
                                ...
                                :key="barista.id"
                                class="transition hover:bg-amber-50/10"
                            >
                                <td class="p-6 font-extrabold text-[#3B2314]">
                                    {{ barista.name }}
                                </td>
                                ...
                                <td class="p-6 text-xs text-gray-400">
                                    {{ formatLocalDate(barista.created_at) }}
                                </td>
                                <td class="p-6">
                                    <div
                                        ...
                                        >
                                            ...
                                            >
                                                <path
                                                    stroke-linecap="round"
                                                    stroke-linejoin="round"
                                                    d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.83 17.93a4.5 4.5 0 0 1-2.22 1.14l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-2.22L16.862 4.487Zm0 0L19.5 7.125"
                                                />
                                            </svg>
                                            Ubah
                                        </button>
        ...
        >
            ...
            >
                <div
                    class="flex items-center justify-between bg-[#3B2314] p-6 text-[#FAEDCD]"
                >
                    <div>
                        <h3 class="text-md font-extrabold tracking-wide">
                            Pendaftaran Barista Baru
                        </h3>
                        <p class="mt-0.5 text-[10px] text-gray-300">
                            Daftarkan akun staf baru untuk mengoperasikan kasir
                            POS.
                        </p>
                    </div>
                    <button
                        @click="closeAddModal"
                        class="text-[#FAEDCD]/60 transition hover:text-white"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
        ...
        <!-- MODAL: UBAH BARISTA -->
        <div
            v-if="showEditModal && selectedBarista"
            class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm"
        >
            <div
                class="w-full max-w-md overflow-hidden rounded-xl border border-[#D4A373]/20 bg-white shadow-2xl transition-all duration-300"
            >
                ...
                >
                    <div>
                        <h3 class="text-md font-extrabold tracking-wide">
                            Ubah Informasi Barista
                        </h3>
                        <p class="mt-0.5 text-[10px] text-gray-300">
                            Ubah rincian profil staf barista kafe Anda.
                        </p>
                    </div>
                    <button
                ...
                <div class="space-y-4 p-6">
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center justify-between">
                            <label
                                class="block text-xs font-extrabold tracking-wider text-[#3B2314] uppercase"
                                >Kata Sandi Baru (Opsional)</label
                            >
        ...
        <div
            ...
            class="animate-fade-in fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm"
        >
            <div
                class="w-full max-w-sm overflow-hidden rounded-xl border border-red-100 bg-white shadow-2xl transition-all duration-300"
            >
                <div class="flex items-center gap-3 bg-red-600 p-6 text-white">
                    <div
                    ...
                    <div>
                        <h4
                            class="text-sm font-extrabold tracking-wide uppercase"
                        >
                            {{ confirmTitle }}
                        </h4>
                    </div>
                ...
                <div class="p-6 text-xs leading-relaxed text-gray-600">
                    {{ confirmMessage }}
                </div>
                ...
                >
                    <button
                        ...
                        class="rounded-xl bg-gray-200 px-4 py-2 text-xs font-bold text-gray-700 transition hover:bg-gray-300"
                    >
                        Batal
                    </button>
                    <button
                        @click="handleConfirmYes"
                        class="rounded-xl bg-red-600 px-4 py-2 text-xs font-bold text-white shadow-md transition hover:bg-red-700"
                    >
                        Hapus Akun
                    </button>
                </div>
