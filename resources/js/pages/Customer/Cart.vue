...
const applyVoucher = async () => {
    ...
    try {
        ...
        if (response.data.success) {
            ...
            form.voucher_code = response.data.voucher.code;
            form.discount_amount = response.data.discount_amount;
            voucherError.value = '';
        }
    } catch (error) {
        console.error(error);
...
const removeCartItem = (index) => {
    ...
    localStorage.setItem('zunoi_cart', JSON.stringify(form.cart_items));
};
...
const editCartItem = (index) => {
    const item = form.cart_items[index];
...
return;
    ...
    const item = form.cart_items[editingIndex.value];
